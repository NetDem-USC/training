...
The following line downloads the ~200 most recent posts on the facebook page of Barack Obama
```{r}
page <- getPage("barackobama", token=fb_oauth, n=20, reactions=TRUE) 
```
...
We can also subset by date. For example, imagine we want to get all the posts from early November 2012 on this Facebook page
```{r}
page <- getPage("barackobama", token=fb_oauth, n=100,
	since='2012/11/01', until='2012/11/10', reactions=TRUE)
page[which.max(page$likes_count),]
